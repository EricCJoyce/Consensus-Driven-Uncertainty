...
from Dataset_NObjNGrip import Dataset_NObjNGrip
...
def main():
	...
	if params['principal-estimator'] is None or params['objects-set-name'] is None or params['grippers-set-name'] is None or params['helpme']:
		usage()
		return

	prod = [params['object-strs'], params['gripper-strs']]			#  Prepare to get Cartesian product.
																	#  Identify mode.
	training_data = Dataset_NObjNGrip( [os.path.join('..', 'DataSplit', params['mode-str'], \
	                                                 '-'.join(['train', params['principal-estimator'], x[0], x[1], params['mode-str']])+'.txt') \
	                                    for x in product(*prod)] )
	...
	test_data = Dataset_NObjNGrip( [os.path.join('..', 'DataSplit', params['mode-str'], \
	                                             '-'.join(['test', params['principal-estimator'], x[0], x[1], params['mode-str']])+'.txt') \
	                                for x in product(*prod)] )
	...
	if params['resume']:											#  Resuming from a previous checkpoint?
		model_checkpoints = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str']])) and x.endswith('.pth')]
		model_checkpoints = sorted(model_checkpoints, key=lambda x: int(x.split('-')[2].split('.')[0]))
																	#  Collect all records, too.
		record_checkpoints = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str']])) and x.endswith('.txt')]
	...
	for t in range(params['initial-epoch'], params['epochs']):		#  Training loop.
		...
		if params['report-everything']:
			...
			model_name = '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth'
		...
		fh = open('-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], str(t).rjust(3, '0')]) + '.txt', 'w')
		...
		fh.write('#  GRIPPER(S): {'+','.join(params['gripper-strs'])+'}\n')
		fh.write('#  GRIPPER.SET: '+params['grippers-set-name']+'\n')
		...
		if (curr_training_loss < least_training_loss and curr_testing_loss < least_testing_loss):
			...
			best_model = '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth'
																	#  Save a copy of the model.
			if params['save-policy'] == 'improve' or params['save-policy'] == 'all':
				torch.save(model.state_dict(), '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth')
				print(textcolors.HEADER + f"Saved model " + '"' + '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth"' + textcolors.ENDC + '\n')
			else:
		...
		if curr_testing_loss < least_testing_loss:
			...
			best_model_test = '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth'
	...
	if params['report-everything']:									#  A confusion matrix and test-set performance already exist. Clone them.
		shutil.copy(best_model[:-4]+'-confusionmatrix.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], 'best', 'confusionmatrix']) + '.txt')
		shutil.copy(best_model[:-4]+'-record.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], 'best', 'record']) + '.txt')
	...
	if params['report-everything']:									#  A confusion matrix and test-set performance already exist. Clone them.
		shutil.copy(best_model_test[:-4]+'-confusionmatrix.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], 'bestGeneralizing', 'confusionmatrix']) + '.txt')
		shutil.copy(best_model_test[:-4]+'-record.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str'], 'bestGeneralizing', 'record']) + '.txt')
	...
	if params['plot-loss']:
		...
		bookmarks = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str']])) \
		...
		records = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['objects-set-name'], params['grippers-set-name'], params['mode-str']])) \
...
def test_and_report(dataloader, model, loss_fn, device, params, network_name, dataset):
	...
	fh.write('#    Raw prediction <t> Rounded prediction <t> Ground-truth label. <t> Dataset <t> Scene <t> Frame <t> Object-name <t> Visibility <t> Gripper\n')
	...
	fh_confmat.write('#  OBJ.SET:     '+params['grippers-set-name']+'\n')
	fh_confmat.write('#  GRIPPER(S): {'+','.join(params['gripper-strs'])+'}\n')
	fh_confmat.write('#  GRIPPER.SET  '+params['grippers-set-name']+'\n')
...
def get_command_line_params():
	...
	params['objects'] = []
	params['object-strs'] = []
	params['objects-set-name'] = None
	params['bop-objects'] = ['LMO1', 'LMO5', 'LMO6', 'LMO8', 'LMO9', 'LMO10', 'LMO11', 'LMO12', 'YCBV2', 'YCBV3', 'YCBV4', 'YCBV5', 'YCBV8', 'YCBV9', 'YCBV12']

	params['grippers'] = []
	params['gripper-strs'] = []
	params['grippers-set-name'] = None								#  Be able to refer to the set of grippers you use. e.g. "both".
	...
	flags = ['-principal', '-objname', '-obj', '-gripname', '-gripper', '-mode', '-epoch', '-epochs', '-optim', \
			...
			if argtarget is not None:
				if argtarget == '-principal':
					...
					if argval.upper().replace('-', '') == 'LMO1':
						...
						params['object-strs'].append( argval.upper().replace('-', '') )
				elif argtarget == '-gripname':
					params['grippers-set-name'] = argval
				elif argtarget == '-gripper':
					if argval.lower()[:3] == 'par':
						params['grippers'].append( Gripper.PARALLEL )
						params['gripper-strs'].append( 'Parallel' )
					elif argval.lower()[:3] == 'und':
						params['grippers'].append( Gripper.UNDERACTUATED )
						params['gripper-strs'].append( 'Underactuated' )
				elif argtarget == '-mode':
...
def usage():
	print('Train for a single (principal, {objects}, {grippers}); test on that same (principal, {objects}, {grippers}):')
	print('============================================================================================================')
	print('python3 train_NObjNGrip.py -principal EPOS -gripname both -gripper par -gripper und -objname all -obj LMO1 -obj LMO5 -obj LMO6 -obj LMO8 -obj LMO9 -obj LMO10 -obj LMO11 -obj LMO12 -obj YCBV2 -obj YCBV3 -obj YCBV4 -obj YCBV5 -obj YCBV8 -obj YCBV9 -obj YCBV12 -epochs 400 -lr 0.0001 -batch 16 -mode t3r3 -v -plot -report -save all')
	print('python3 train_NObjNGrip.py -principal GDRNPP -gripname both -gripper par -gripper und -objname all -obj LMO1 -obj LMO5 -obj LMO6 -obj LMO8 -obj LMO9 -obj LMO10 -obj LMO11 -obj LMO12 -obj YCBV2 -obj YCBV3 -obj YCBV4 -obj YCBV5 -obj YCBV8 -obj YCBV9 -obj YCBV12 -epochs 400 -lr 0.0001 -batch 16 -mode t3r3 -v -plot -report -save all')
	print('python3 train_NObjNGrip.py -principal ZebraPose -gripname both -gripper par -gripper und -objname all -obj LMO1 -obj LMO5 -obj LMO6 -obj LMO8 -obj LMO9 -obj LMO10 -obj LMO11 -obj LMO12 -obj YCBV2 -obj YCBV3 -obj YCBV4 -obj YCBV5 -obj YCBV8 -obj YCBV9 -obj YCBV12 -epochs 400 -lr 0.0001 -batch 16 -mode t3r3 -v -plot -report -save all')
	return
