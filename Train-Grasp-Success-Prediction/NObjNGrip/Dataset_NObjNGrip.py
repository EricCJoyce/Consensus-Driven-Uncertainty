...
'''
Class for a dataset for several objects and both grippers.
'''
class Dataset_NObjNGrip(Dataset):
	def __init__(self, files):
		...
		self.grippers_encountered = []								#  Store unique gripper strings, as taken from file names.

		max_prog_bar_len = os.get_terminal_size().columns			#  Total width of the terminal.

		for file in files:											#  FIRST pass: encounter ALL GRIPPERS IN SET DETERMINED BY "files".
			...
			if gripper not in self.grippers_encountered:
				self.grippers_encountered.append( gripper )			#  Mark as recognized. In the order given.

		self.obj_one_hot_length = len(self.objects_encountered)		#  Lock down the length of the OBJECTS' one-hot sub-vector.
		self.gripper_one_hot_length = len(self.grippers_encountered)#  Lock down the length of the GRIPPERS' one-hot sub-vector.
		...
		for file in files:											#  SECOND pass: encounter ALL OBJECTS IN SET DETERMINED BY "files".
			...
			for line in lines:
				...
				self.input_length = len(principal_support_differences) + self.obj_one_hot_length + self.gripper_one_hot_length

				obj_one_hot_vector = [0.0 for i in range(0, self.obj_one_hot_length)]
				obj_one_hot_vector[ self.objects_encountered.index( obj ) ] = 1.0

				gripper_one_hot_vector = [0.0 for i in range(0, self.gripper_one_hot_length)]
				gripper_one_hot_vector[ self.grippers_encountered.index( gripper ) ] = 1.0

																	#  To be reshaped (batch-size x input_length).
				X.append( principal_support_differences + obj_one_hot_vector + gripper_one_hot_vector )
