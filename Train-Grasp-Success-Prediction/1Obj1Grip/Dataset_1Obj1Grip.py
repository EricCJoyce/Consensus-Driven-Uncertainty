...
'''
Class for a dataset for a single object and a single gripper.
'''
class Dataset_1Obj1Grip(Dataset):
	def __init__(self, file):
		...
		mode = arr[4]												#  Always [4], in {t1r1, t3r3, t1r3, t3r1, add}.
		...
		for line in lines:
			...
			self.input_length = len(principal_support_differences)

			X.append( principal_support_differences )				#  To be reshaped (batch-size x input_length).
			y.append( success )										#  To be reshaped (batch-size x 1).
																	#  Save reference, so we can look up the source for this sample.
			self.src.append( (dataset, scene, frame, local_id, visibility, gripper) )
