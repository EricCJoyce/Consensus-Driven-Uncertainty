...
from Dataset_1Obj1Grip import Dataset_1Obj1Grip
...
def main():
	...
	if params['principal-estimator'] is None or params['helpme']:
		usage()
		return
																	#  Identify mode.
	training_data = Dataset_1Obj1Grip( os.path.join('..', 'DataSplit', params['mode-str'], \
	                                                '-'.join(['train', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str']])+'.txt') )
	...
	test_data = Dataset_1Obj1Grip( os.path.join('..', 'DataSplit', params['mode-str'], \
	                                            '-'.join(['test', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str']])+'.txt') )
	...
	if params['resume']:											#  Resuming from a previous checkpoint?
		model_checkpoints = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str']])) and x.endswith('.pth')]
		model_checkpoints = sorted(model_checkpoints, key=lambda x: int(x.split('-')[2].split('.')[0]))
																	#  Collect all records, too.
		record_checkpoints = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str']])) and x.endswith('.txt')]
	...
	for t in range(params['initial-epoch'], params['epochs']):		#  Training loop.
		...
		if params['report-everything']:
			...
			model_name = '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth'
		...
		fh = open('-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], str(t).rjust(3, '0')]) + '.txt', 'w')
		...
		if (curr_training_loss < least_training_loss and curr_testing_loss < least_testing_loss):
			...
			best_model = '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth'
																	#  Save a copy of the model.
			if params['save-policy'] == 'improve' or params['save-policy'] == 'all':
				torch.save(model.state_dict(), '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth')
				print(textcolors.HEADER + f"Saved model " + '"' + '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth"' + textcolors.ENDC + '\n')
			else:
		...
		if curr_testing_loss < least_testing_loss:
			...
			best_model_test = '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], str(t).rjust(3, '0')]) + '.pth'
	...
	if params['report-everything']:									#  A confusion matrix and test-set performance already exist. Clone them.
		shutil.copy(best_model[:-4]+'-confusionmatrix.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], 'best', 'confusionmatrix']) + '.txt')
		shutil.copy(best_model[:-4]+'-record.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], 'best', 'record']) + '.txt')
	...
	if params['report-everything']:									#  A confusion matrix and test-set performance already exist. Clone them.
		shutil.copy(best_model_test[:-4]+'-confusionmatrix.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], 'bestGeneralizing', 'confusionmatrix']) + '.txt')
		shutil.copy(best_model_test[:-4]+'-record.txt', \
		            '-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str'], 'bestGeneralizing', 'record']) + '.txt')
	...
	if params['plot-loss']:
		...
		bookmarks = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str']])) \
		...
		records = [x for x in os.listdir() if x.startswith('-'.join(['GraspSuccess', params['principal-estimator'], params['object-str'], params['gripper-str'], params['mode-str']])) \
...
def get_command_line_params():
	...
	flags = ['-principal', '-object', '-gripper', '-mode', '-epoch', '-epochs', '-optim', \
...
def usage():
	print('Train for a single (principal, object, gripper); test on that same (principal, object, gripper):')
	print('================================================================================================')
	print('python3 train_1Obj1Grip.py -principal EPOS -object LMO1 -gripper par -epochs 400 -lr 0.0001 -batch 16 -mode t3r3 -v -plot -report -save all')
	print('python3 train_1Obj1Grip.py -principal GDRNPP -object LMO1 -gripper par -epochs 400 -lr 0.0001 -batch 16 -mode t3r3 -v -plot -report -save all')
	print('python3 train_1Obj1Grip.py -principal ZebraPose -object LMO1 -gripper par -epochs 400 -lr 0.0001 -batch 16 -mode t3r3 -v -plot -report -save all')
	return
