...
def main():
	...
	fair_game    = ['ape', 'can', 'cat', 'driller', 'duck', 'eggbox', 'glue', 'holepuncher']
	root_path    = '/media/eric/Hoboken/Projects/Uncertainty for 6DoF Pose Est/Datasets/BOP/'
	dataset_path = 'lmo/'
	set_path     = 'test/'

	fh_err = open('lmo_pose_error.txt', 'w')
	fh_err.write('#  Pose errors for ZebraPose on LMO.\n')
	...
	json_models = json.load(fh)										#  Keys are integers as strings: "1", "5", "6", "8", "9", "10", "11", "12"
	fh.close()														#  Check if 'symmetries_continuous' in json_models[obj_str]
																	#     or if 'symmetries_discrete' in json_models[obj_str].
	for scene in [str(x).rjust(6, '0') for x in range(2, 3)]:		#  For every scene in the test set...
		if verbose:
			print('>>> Scene ' + scene)
		...
		for keyframe in keyframes:									#  NOTE for YCBV: we may assume that there is only one instance of any present object,
			...
			for obj in fair_game:									#                                            in pixel coords}
				fh = open('evaluation_output/pose_result_bop/lmo/' + dataset_path[:-1] + '_' + obj + '.csv', 'r')
				lines_csv = fh.readlines()[1:]						#  Skip header.
				fh.close()

				fh = open('evaluation_output/pose_result_bop/lmo/' + dataset_path[:-1] + '_' + obj + '.bbox', 'r')
...
def lookup_class_name(class_id):
	if class_id == 1:												#  Red ape figure
		return 'ape'
	elif class_id == 5:												#  Watering can
		return 'can'
	elif class_id == 6:												#  Pink cat figure
		return 'cat'
	elif class_id == 8:												#  Drill
		return 'driller'
	elif class_id == 9:												#  Yellow duck figure
		return 'duck'
	elif class_id == 10:											#  Egg carton
		return 'eggbox'
	elif class_id == 11:											#  White bottle of glue
		return 'glue'
	elif class_id == 12:											#  Blue desktop hole puncher
		return 'holepuncher'
...
def lookup_class_id(class_name):
	if class_name == 'ape':
		...
		return 6
	elif class_name == 'driller':
		return 8
	elif class_name == 'duck':
		return 9
	elif class_name == 'eggbox':
		return 10
	elif class_name == 'glue':
		return 11
	elif class_name == 'holepuncher':
		return 12
