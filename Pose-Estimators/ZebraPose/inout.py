...
def load_ply(path):
  ...
  #model['pts'] = np.zeros((n_pts, 3), np.float)                    #  EJ
  ...
  if {'nx', 'ny', 'nz'}.issubset(set(pt_props_names)):
    is_normal = True
    #model['normals'] = np.zeros((n_pts, 3), np.float)              #  EJ
    model['normals'] = np.zeros((n_pts, 3), np.float64)

  is_color = False
  if {'red', 'green', 'blue'}.issubset(set(pt_props_names)):
    is_color = True
    #model['colors'] = np.zeros((n_pts, 3), np.float)               #  EJ
    model['colors'] = np.zeros((n_pts, 3), np.float64)

  is_texture_pt = False
  if {'texture_u', 'texture_v'}.issubset(set(pt_props_names)):
    is_texture_pt = True
    #model['texture_uv'] = np.zeros((n_pts, 2), np.float)           #  EJ
  ...
  if {'texcoord'}.issubset(set(face_props_names)):
    ...
    #model['texture_uv_face'] = np.zeros((n_faces, 6), np.float)    #  EJ
