...
def main(configs):
    ...
    if 'detector' not in configs:                                   #  EJ
        #configs['detector'] = None
        configs['detector'] = 'FCOS'
    ...
    for batch_idx, (data, entire_masks, masks, Rs, ts, Bboxes, class_code_images, cam_Ks) in enumerate(tqdm(test_loader)):
        ...
        for counter, (r_GT, t_GT, Bbox, cam_K) in enumerate(zip(Rs, ts, Bboxes, cam_Ks)):
            ...
            if success:     
                if configs['use_icp']:                              #  EJ: When NOT using ICP, R_predict = R_predict and t_predict = t_predict.
    ...
    if not os.path.exists(cvs_path):
        os.makedirs(cvs_path)
                                                                    #  EJ: Save bounding box details.
    fh = open(cvs_path + "{}_{}".format(dataset_name, obj_name) + '.bbox', 'w')
    for det_bbox in Det_Bbox:
        if det_bbox is not None:
            fh.write(' '.join([str(x) for x in det_bbox]) + '\n')
    fh.close()                                                      #  EJ: We'll reunite these with their pose predictions later.
