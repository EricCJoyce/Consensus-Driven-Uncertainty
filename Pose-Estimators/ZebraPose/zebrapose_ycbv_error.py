...
def main():
	...
	fair_game    = ['master_chef_can', 'cracker_box', 'sugar_box', 'tomato_soup_can', 'mustard_bottle', 'tuna_fish_can', \
	                'pudding_box', 'gelatin_box', 'potted_meat_can', 'banana', 'pitcher_base', 'bleach_cleanser', 'bowl', 'mug', \
	                'power_drill', 'wood_block', 'scissors', 'large_marker', 'large_clamp', 'extra_large_clamp', 'foam_brick']
	root_path    = '/media/eric/Hoboken/Projects/Uncertainty for 6DoF Pose Est/Datasets/BOP/'
	dataset_path = 'ycbv/'
	set_path     = 'test/'

	fh_err = open('ycbv_pose_error.txt', 'w')
	fh_err.write('#  Pose errors for ZebraPose on YCB-V.\n')
	fh_err.write('#  Scene-number  <t>  Keyframe-number  <t>  Object  <t>  Visibility-ratio  <t>  Gr.Tr.Rotation(space-sep, 9)  <t>  Gr.Tr.Translation(space-sep, 3)  <t>  Est.Rotation(space-sep, 9)  <t>  Est.Translation(space-sep, 3)  <t>  Rotation-error(degrees)  <t>  Translation-error(mm)\n')
																	#  YCBV has only a few objects requiring that we consider symmetries to evaluate fairly.
	fh = open(root_path + dataset_path + 'models/models_info.json', 'r')
	json_models = json.load(fh)										#  Keys are integers as strings: "1" through "21"
	fh.close()														#  Check if 'symmetries_continuous' in json_models[obj_str]
																	#     or if 'symmetries_discrete' in json_models[obj_str].
	for scene in [str(x).rjust(6, '0') for x in range(48, 60)]:		#  For every scene in the test set...
		if verbose:
			print('>>> Scene ' + scene)
		...
		keyframes = json_gt.keys()									#  For YCB-V only these keyframes have ground truth.

		for keyframe in keyframes:									#  NOTE for YCBV: we may assume that there is only one instance of any present object,
			if verbose:												#  so this script is more robust than it needs to be.
			...
			for obj in fair_game:									#                                            in pixel coords}
				fh = open('evaluation_output/pose_result_bop/ycbv/' + dataset_path[:-1] + '_' + obj + '.csv', 'r')
				lines_csv = fh.readlines()[1:]						#  Skip header.
				fh.close()

				fh = open('evaluation_output/pose_result_bop/ycbv/' + dataset_path[:-1] + '_' + obj + '.bbox', 'r')
...
def lookup_class_name(class_id):
	if class_id == 1:												#  Master Chef, blue coffee grind can
		return 'master_chef_can'
	elif class_id == 2:												#  Cheez It, red cracker box
		return 'cracker_box'
	elif class_id == 3:												#  Domino Sugar, yellow box
		return 'sugar_box'
	elif class_id == 4:												#  Campbell's tomato soup
		return 'tomato_soup_can'
	elif class_id == 5:												#  French's yellow mustard squeeze bottle
		return 'mustard_bottle'
	elif class_id == 6:												#  Starkist tuna, blue tin
		return 'tuna_fish_can'
	elif class_id == 7:												#  JELL-O chocolate instant pudding box
		return 'pudding_box'
	elif class_id == 8:												#  JELL-O strawberry instant gelatin box
		return 'gelatin_box'
	elif class_id == 9:												#  SPAM tin
		return 'potted_meat_can'
	elif class_id == 10:											#  Banana
		return 'banana'
	elif class_id == 11:											#  Blue plastic pitcher with grip-moulded lid
		return 'pitcher_base'
	elif class_id == 12:											#  Soft Scrub bleach, white squeeze bottle
		return 'bleach_cleanser'
	elif class_id == 13:											#  Red enamelware bowl
		return 'bowl'
	elif class_id == 14:											#  Red enamelware mug
		return 'mug'
	elif class_id == 15:											#  Black+Decker, orange drill
		return 'power_drill'
	elif class_id == 16:											#  Block of wood
		return 'wood_block'
	elif class_id == 17:											#  Westcott scissors, gray and maize handle
		return 'scissors'
	elif class_id == 18:											#  Expo dry erase marker, black (and white on body)
		return 'large_marker'
	elif class_id == 19:											#  Workbench clamp, black with orange tips
		return 'large_clamp'
	elif class_id == 20:											#  [same]
		return 'extra_large_clamp'
	elif class_id == 21:											#  Maroon block with three brick-like holes in top, but not through
		return 'foam_brick'
	return None														#  Should never happen.

def lookup_class_id(class_name):
	if class_name == 'master_chef_can':								#  Master Chef, blue coffee grind can
		return 1
	elif class_name == 'cracker_box':								#  Cheez It, red cracker box
		return 2
	elif class_name == 'sugar_box':									#  Domino Sugar, yellow box
		return 3
	elif class_name == 'tomato_soup_can':							#  Campbell's tomato soup
		return 4
	elif class_name == 'mustard_bottle':							#  French's yellow mustard squeeze bottle
		return 5
	elif class_name == 'tuna_fish_can':								#  Starkist tuna, blue tin
		return 6
	elif class_name == 'pudding_box':								#  JELL-O chocolate instant pudding box
		return 7
	elif class_name == 'gelatin_box':								#  JELL-O strawberry instant gelatin box
		return 8
	elif class_name == 'potted_meat_can':							#  SPAM tin
		return 9
	elif class_name == 'banana':									#  Banana
		return 10
	elif class_name == 'pitcher_base':								#  Blue plastic pitcher with grip-moulded lid
		return 11
	elif class_name == 'bleach_cleanser':							#  Soft Scrub bleach, white squeeze bottle
		return 12
	elif class_name == 'bowl':										#  Red enamelware bowl
		return 13
	elif class_name == 'mug':										#  Red enamelware mug
		return 14
	elif class_name == 'power_drill':								#  Black+Decker, orange drill
		return 15
	elif class_name == 'wood_block':								#  Block of wood
		return 16
	elif class_name == 'scissors':									#  Westcott scissors, gray and maize handle
		return 17
	elif class_name == 'large_marker':								#  Expo dry erase marker, black (and white on body)
		return 18
	elif class_name == 'large_clamp':								#  Workbench clamp, black with orange tips
		return 19
	elif class_name == 'extra_large_clamp':							#  [same, but bigger]
		return 20
	elif class_name == 'foam_brick':								#  Maroon block with three brick-like holes in top, but not through
		return 21
