...
def main():
	...
	fair_game    = ['obj' + str(x).rjust(2, '0') for x in range(1, 31)]
	root_path    = '/media/eric/Hoboken/Projects/Uncertainty for 6DoF Pose Est/Datasets/BOP/'
	dataset_path = 'tless/'
	set_path     = 'test_primesense_bop/'

	fh_err = open('tless_pose_error.txt', 'w')
	fh_err.write('#  Pose errors for ZebraPose on TLESS.\n')
	...
	fh = open(root_path + dataset_path + 'models_cad/models_info.json', 'r')
	json_models = json.load(fh)										#  Keys are integers as strings: obj01 ==> "1"; obj30 ==> "30"
	fh.close()														#  Check if 'symmetries_continuous' in json_models[obj_str]
																	#     or if 'symmetries_discrete' in json_models[obj_str].
	for scene in [str(x).rjust(6, '0') for x in range(1, 21)]:		#  For every scene in the test set...
		if verbose:
			print('>>> Scene ' + scene)
		...
		for keyframe in keyframes:									#  NOTE for TLESS: We CANNOT assume that there is only one instance of any present object!
			...
			for obj in fair_game:									#                                            in pixel coords}
				fh = open('evaluation_output/pose_result_bop/tless/' + dataset_path[:-1] + '_' + obj + '.csv', 'r')
				lines_csv = fh.readlines()[1:]						#  Skip header.
				fh.close()

				fh = open('evaluation_output/pose_result_bop/tless/' + dataset_path[:-1] + '_' + obj + '.bbox', 'r')
